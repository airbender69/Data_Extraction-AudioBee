...
scraper.py
==========
Playwright scraper for ekantipur.com
Extracts:
...
def make_absolute(url: str) -> str:
    """Turn a relative URL into a full absolute URL."""
    if not url:
        return None
    url = url.strip()
    if url.startswith("//"):
...
def safe_text(element, selector: str) -> str | None:
    ...
    Returns None instead of crashing if the element doesn't exist.
    """
    try:
...
def safe_attr(element, selector: str, attr: str) -> str | None:
    """
    Safely get an attribute value from a child element.
    Returns None instead of crashing if the element doesn't exist.
    """
    try:
        el = element.query_selector(selector)
...
def goto_page(page, url: str) -> bool:
    """
    Navigate to a URL. Uses 'commit' so we don't wait for the full
    page to load — just wait for the first server response, then
    pause 5 seconds for JavaScript content to render.
...
def scroll_to_load(page):
    """
    Slowly scroll down the page so lazy-loaded images and content appear.
    Many news sites only load images when you scroll near them.
    """
    for i in range(1, 5):
...
def extract_entertainment_news(page) -> list[dict]:
    """
    Go to ekantipur.com/entertainment and extract the top 5 articles.

    From debug output we know the real HTML structure is:
      div.category-wrapper
        div.category              ← one article card
          div.category-inner-wrapper
            div.category-description
              h2 > a              ← title (text + href)
              div.author-name > p > a  ← author name
            div.category-image
              figure > img        ← image (src attribute)
    """
